...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.nl = len(ch)  # number of detection layers
        self.reg_max = 16  # DFL channels (ch[0] // 16 to scale 4/8/12/16/20 for n/s/m/l/x)
        self.no = 2 * nc + self.reg_max * 4  # number of outputs per anchor (add channel-wise mass)
        self.stride = torch.zeros(self.nl)  # strides computed during build
        c2, c3, c4 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100)), max(ch[0], min(self.nc, 100))  # channels
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
        )
        ...
        self.cv4 = nn.ModuleList(
            nn.Sequential(Conv(x, c4, 3), Conv(c4, c4, 3), nn.Conv2d(c4, self.nc, 1)) for x in ch
        )
    ...
    def forward(self, x):
        ...
        if self.training or self.export:  # Training path
            return x
        y = self._inference(x)
        return y if self.export else (y, x)
    ...
    def _inference(self, x):
        ...
        if self.export and self.format in {"saved_model", "pb", "tflite", "edgetpu", "tfjs"}:  # avoid TF FlexSplitV ops
            ...
            mass = x_cat[:, self.reg_max * 4 + self.nc :]
        else:
            box, cls, mass = x_cat.split((self.reg_max * 4, self.nc, self.nc), 1) # splits to get bbox dfl/distribution features of bbox in (ltrb) format and cls probs
        
        '''
        max_cls_idx = cls.argmax(dim=1, keepdim=True) # gets the max idx of max cls prob
        max_mass = mass.gather(1, max_cls_idx).relu() # gets the mass of max cls prob idx, make neg values 0
        '''
        if self.export and self.format in {"tflite", "edgetpu"}:
            # Precompute normalization factor to increase numerical stability
            # See https://github.com/ultralytics/ultralytics/issues/7371
            ...
            return dbox.transpose(1, 2), cls.sigmoid().permute(0, 2, 1), mass.permute(0, 2, 1)
        else:
            dbox = self.decode_bboxes(self.dfl(box), self.anchors.unsqueeze(0)) * self.strides # gets boxes in xywh format after dfl conversion to single ltrb values
...
# Model does not support segmentation yet
