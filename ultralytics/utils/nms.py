...
def non_max_suppression(
    ...
    t = time.time()
    output = [torch.zeros((0, 6 + nm + 1), device=prediction.device)] * bs # added mass
    keepi = [torch.zeros((0, 1), device=prediction.device)] * bs  # to store the kept idxs
    ...
    for xi, (x, xk) in enumerate(zip(prediction, xinds)):  # image index, (preds, preds indices)
        ...
        if labels and len(labels[xi]) and not rotated:
            lb = labels[xi]
            v = torch.zeros((len(lb), (2*nc) + nm + 4), device=x.device) # include mass
            v[:, :4] = xywh2xyxy(lb[:, 1:5])  # box
            v[range(len(lb)), lb[:, 0].long() + 4] = 1.0  # cls
            x = torch.cat((x, v), 0)
        ...
        # Detections matrix nx6 (xyxy, conf, cls)
        box, cls, mask, mass = x.split((4, nc, nm, nc), 1)

        if multi_label:
            i, j = torch.where(cls > conf_thres)
            x = torch.cat((box[i], x[i, 4 + j, None], j[:, None].float(), mask[i], mass[i, j, None]), 1)
            if return_idxs:
                xk = xk[i]
            ...
            conf, j = cls.max(1, keepdim=True)
            filt = conf.view(-1) > conf_thres
            x = torch.cat((box, conf, j.float(), mask, mass.gather(1, j)), 1)[filt]
            if return_idxs:
                xk = xk[filt]
        ...
        if not n:  # no boxes
            continue
        if n > max_nms:  # excess boxes
            filt = x[x[:, 4].argsort(descending=True)[:max_nms]]  # sort by confidence and remove excess boxes
            x = x[filt]
            if return_idxs:
                xk = xk[filt]
